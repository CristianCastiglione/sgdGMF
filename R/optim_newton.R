...
gmf.newton = function (
  ...
  control = set.control("newton", control)
  ...
  for (t in 1:maxiter) {
    ...
    dratio = family$mu.eta(eta) / family$variance(mu)
    ddratio = family$mu.eta(eta) * dratio
    ddiff = (Y - mu) * dratio

    # gaussian()$mu.eta returns a vector instead of a matrix
    # potentially, other families too, so we add the following umbrella conversion
    if (!is.matrix(dratio)) dratio = matrix(dratio, nrow = n, ncol = m)
    if (!is.matrix(ddratio)) ddratio = matrix(ddratio, nrow = n, ncol = m)
    if (!is.matrix(ddiff)) ddiff = matrix(ddiff, nrow = n, ncol = m)

    # Update model parameters, updating U and V at once
    Ut = update.newton(U[, idu, drop = FALSE], V[, idu, drop = FALSE],
                       penalty = pu[idu], ddiff = ddiff, ddratio = ddratio,
                       stepsize = stepsize, damping = damping)
    Vt = update.newton(V[, idv, drop = FALSE], U[, idv, drop = FALSE],
                       penalty = pv[idv], ddiff = t(ddiff), ddratio = t(ddratio),
                       stepsize = stepsize, damping = damping)
