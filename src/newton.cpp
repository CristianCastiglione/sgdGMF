...
Rcpp::List Newton::fit (
    ...
    set_data_bounds(mulo, muup, etalo, etaup, this->eps, ymin, ymax, family);
    ...
    for (iter = 1; iter < this->maxiter; iter++) {
        ...
        this->update(ut, v, penu, idu, deta, ddeta, this->stepsize, this->damping);
        this->update(vt, u, penv, idv, deta.t(), ddeta.t(), this->stepsize, this->damping);
        u = ut;
        v = vt;
        ...
        diter = static_cast<double>(iter);
        trace.row(iter) = arma::vec{diter, dev, pen, obj, change, time};
