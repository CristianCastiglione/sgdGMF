...
Then, we load the ant traits data in the workspace and we define the response and covariate matrices.
...
`sgdGMF` is an package designed to efficiently estimate high-dimensional GMF models using either deterministic or stochastic optimization algorithms. 
In general, GMF models extend generalized linear models to matrix-variate data.
As such, we assume that the response variable $y_{ij} \in \mathcal{Y}$ has as conditional distribution of the form
\[
...
To this aim, we can just use the standard family builders used by the `glm` function.
...
The `sgdGMF.rank` function first estimate a vector generalized linear model, corresponding to the case $u_i^\top v_j = 0$, then extract the covariance matrix of the deviance residuals of the model and, finally, find the elbow of the covariance screeplot using the specified thresholding method.
Additionally, we set some return parameters as `TRUE` to save some useful information that will be analyzed later on.
...
The default optimizer in `sgdgmf.fit` is `method = "airwls"`, which uses alternated iterative re-weighted least square.
Instead, we here set `method = "bsgd"`, which employs the stochastic gradient descent method with block-subsampling strategy proposed in Castiglione, et al. (2024, arXiv).

## Model validation

Let us now check the model performance.
...
If we observe a elbow in the plot, this suggest that probably, we are not using the correct number of latent factors.
...
On the other hand, the residual screeplot of the complete GMF model presents a smooth decay of the eigenvalues without an evident elbow, which is good, because this mean that we do not need to introduce additional factors in the matrix factorization.
