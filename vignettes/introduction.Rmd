...
```{r fit}
gmf = sgdgmf.fit(Y, X, Z, ncomp = ncomp, family = family, method = "sgd")
```

The default optimizer in `sgdgmf.fit` is `method = "airwls"`, which uses alternated iterative re-weighted least squares (AIRWLS).
Instead, we here set `method = "sgd"`, which employs the stochastic gradient descent method with block-subsampling strategy proposed in Castiglione, et al. (2024, arXiv).
